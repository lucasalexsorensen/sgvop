...
from ._utils import _get_ndc_rays, _get_ray_directions, _get_rays


########## Slightly modified version of LLFF data loading code
##########  see https://github.com/Fyusion/LLFF for original
def _get_bbox3d_for_llff(poses, hwf, near=0.0, far=1.0):
    H, W, focal = hwf
    H, W = int(H), int(W)
    ...
    for pose in poses:
        rays_o, rays_d = _get_rays(directions, pose)
        rays_o, rays_d = _get_ndc_rays(H, W, focal, 1.0, rays_o, rays_d)
...
def _render_path_spiral(c2w, up, rads, focal, zdelta, zrate, rots, N):
    render_poses = []
    ...
    for theta in np.linspace(0.0, 2.0 * np.pi * rots, N + 1)[:-1]:
        ...
        z = _normalize(c - np.dot(c2w[:3, :4], np.array([0, 0, -focal, 1.0])))
        render_poses.append(np.concatenate([_viewmatrix(z, up, c), hwf], 1))
    return render_poses
...
def _spherify_poses(poses, bds):
    ...
    vec0 = _normalize(up)
    vec1 = _normalize(np.cross([0.1, 0.2, 0.3], vec0))
...
def load_llff_data(
    ...
    if spherify:
        poses, render_poses, bds = _spherify_poses(poses, bds)
        ...
        tt = poses[:, :3, 3]  # _ptstocam(poses[:3,3,:].T, c2w).T
        ...
        render_poses = _render_path_spiral(
